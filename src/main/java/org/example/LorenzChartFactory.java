...
public class LorenzChartFactory implements ChartFactory {
  /**
   * Calculates the Lorenz data for a given DemographicData object. The method takes the income
   * distribution from the DemographicData object and calculates the cumulative households and
   * cumulative income percentages for each income range. The calculated data is returned as a 2D
   * array containing two rows: Row 1: Cumulative share of households percentages Row 2: Cumulative
   * share of income percentages
   *
  ...
  private static double[][] calculateLorenzData(DemographicData data) {
    Map<String, Integer> incomeDistribution = data.incomeDistribution();

    int totalHouseholds = incomeDistribution.values().stream().mapToInt(Integer::intValue).sum();
    double totalIncome =
        incomeDistribution.entrySet().stream()
